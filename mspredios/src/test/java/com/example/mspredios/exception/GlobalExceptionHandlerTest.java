...
public class GlobalExceptionHandlerTest {
    ...
    private static class ExceptionTestController {
        ...
        public void throwNotFound() {
            throw new EntityNotFoundException("Predio no encontrado");
        }

        @GetMapping("/test/predios-exception")
        public void throwPredios() {
            throw new PrediosException("Error en la regla de negocio del predio", 409);
        }
        ...
        public void throwValidation() throws MethodArgumentNotValidException {
            var bindingResult = new BeanPropertyBindingResult(new Object(), "predio");
            bindingResult.addError(new FieldError("predio", "nombre", "El nombre es obligatorio"));
    ...
    void debeManejarNotFound() throws Exception {
        mockMvc.perform(get("/test/404"))
                ...
                .andExpect(jsonPath("$.mensaje").value("Predio no encontrado"))
    ...
    @Test
    @DisplayName("Debe capturar PrediosException y retornar 409 Conflict")
    void debeManejarPrediosException() throws Exception {
        mockMvc.perform(get("/test/predios-exception"))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.status").value(409))
                .andExpect(jsonPath("$.codigo").value("NEGOCIO_ERROR"))
                .andExpect(jsonPath("$.mensaje").value("Error en la regla de negocio del predio"));
    }
